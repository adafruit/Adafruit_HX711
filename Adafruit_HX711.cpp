...
/**
 * @brief Read data from the HX711, handling channel and gain setup, and with 'tare' offset
 * 
 * @param chanGain Channel and gain configuration
 * @return int32_t The signed 32-bit extended raw sensor data
 */
int32_t Adafruit_HX711::readChannel(hx711_chanGain_t chanGain) {
    return readChannelRaw(chanGain) - (chanGain == CHAN_B_GAIN_32 ? _tareValueB : _tareValueA);
}

/**
 * @brief Read data from the HX711, handling channel and gain setup, NO tare offset - the 
 * 'raw' ADC value from the HX!
 * 
 * @param chanGain Channel and gain configuration
 * @return int32_t The signed 32-bit extended raw sensor data
 */
int32_t Adafruit_HX711::readChannelRaw(hx711_chanGain_t chanGain) {
    while (isBusy()); // Wait until the HX711 is ready
